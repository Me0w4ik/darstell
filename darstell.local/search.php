...
if ($search) {
    ...
    $sql = "SELECT images.img, images.name, users.avatar, users.nik FROM images JOIN users ON images.iduser = users.id WHERE images.name LIKE '%$search%' OR images.description LIKE '%$search%' OR images.tags LIKE '%$search%' ORDER BY images.id DESC";
} 
...
<body>
  ...
  <div class="container-menu">
    <div class="searchinfo">
        <h3>По результатам поиска: <?php echo htmlspecialchars($search); ?></h3>
            <div class="results">
                <?php if (empty($images)): ?>
                    <p>Ничего не найдено.</p>
                    <img class="okak" src="/icon/darstell_okak.svg">
                <?php else: ?>
...
<script>
  ...
  images.forEach(image => {
      ...
      nikElement.textContent = image.nik; // Ник пользователя

      const nameElement = document.createElement('p');
      nameElement.className = 'nameimg';
      nameElement.textContent = image.name; // Имя пользователя

      nameElement.textContent = truncateString(image.name, 20);
