...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST["login"]) && isset($_POST["password"])) {
        ...
        $sql = "SELECT * FROM users WHERE login = '$login' AND password2 = '$password'";
    ...
    <main>
      <form class="form" action="login.php" method="post" id="loginform">
        <div class="input-container">
          <input name="login" id="login" class="input" type="login" maxlength="250" placeholder="" />
          <label for="login" class="placeholder">Логин</label>
        </div>
        <div class="input-container">
          <input autocomplete="off" name="password" id="password" class="input" type="password" maxlength="250" placeholder="" />
        ...
        <div class="form-funks">
          <div class="form-reg">
            ...
            <div class="general-palk"></div>
            <a href="register.php" class="form-register">Зарегистрироваться</a>
          </div>
        </div>
      </form>
    </main>
    <main class="info-block" id="fail">
...
$("#loginform").on("submit", function(e) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if (data == 'fail') {
                $('#sussess, #fail, #error').removeClass('show appear');
                $('#fail').addClass('show appear'); // Добавляем классы для анимации
                setTimeout(() => {
                    $('#fail').removeClass('appear'); // Убираем класс анимации после показа
                }, 500); // Время анимации
            }
        },
        error: function() {
            $('#sussess, #fail, #error').removeClass('show appear');
            $('#error').addClass('show appear'); // Добавляем классы для анимации
            setTimeout(() => {
                $('#error').removeClass('appear'); // Убираем класс анимации после показа
            }, 500); // Время анимации
        }
    });
});
